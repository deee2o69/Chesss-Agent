...
class CustomChessEnv(gym.Env):
    def __init__(self, display_moves=False, display_game=False):
    ...
    def step(self, action):
        ...
        stockfish_move = self.engine.play(self.board, chess.engine.Limit(time=MOVETIME)).move
        self.board.push(stockfish_move)
        if self.display_game:
            self.render()  # Print the chessboard if enabled

        # Use Stockfish to evaluate the board
        result = self.engine.analyse(self.board, chess.engine.Limit(time=MOVETIME))
        ...
        if abs(self.reward) > 3 :
            self.reward *= 6
        elif abs(self.reward) > 1 :
            self.reward *= 2
        self.reward = np.clip(self.reward, -100, 100)/100
